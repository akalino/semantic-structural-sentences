...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="train_transd",
                                     description="Trains the TransD algorithm for kg embeddings")
    ...
    if args.data == 'riedel':
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "transd_{d}_riedel.ckpt".format(d=args.dim))
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "transd_{d}_gids.ckpt".format(d=args.dim))
        hyperparams = {'batch_size': 128,
                       'neg_ent': 25,
                       'neg_rel': 0,
                       'margin': 5.0,
    ...
    trainer = Trainer(model=model,
                      ...
                      use_gpu=True)
    trainer.run()
    transd.save_checkpoint(out_path)
    transd.load_checkpoint(out_path)
    tester = Tester(model=transd, data_loader=test_dataloader, use_gpu=True)
    tester.run_link_prediction(type_constrain=True)
    try:
        text_results("Finished training TransD with {n} dimensional vectors on {d} data.".format(n=args.dim,
                                                                                                 d=args.data))
    except:
        print('Done')
