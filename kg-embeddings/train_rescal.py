...
from OpenKE.openke.module.model import RESCAL
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="train_rescal",
                                     description="Trains the RESCAL algorithm for kg embeddings")
    ...
    if args.data == 'riedel':
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "rescal_{d}_riedel.ckpt".format(d=args.dim))
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "rescal_{d}_gids.ckpt".format(d=args.dim))
    ...
    model = NegativeSampling(model=res,
    ...
    trainer = Trainer(model=model,
                      ...
                      use_gpu=True)
    trainer.run()
    res.save_checkpoint(out_path)
    res.load_checkpoint(out_path)
    #tester = Tester(model=res, data_loader=test_dataloader, use_gpu=True)
    #tester.run_link_prediction(type_constrain=True)
    try:
        text_results("Finished training RESCAL with {n} dimensional vectors on {d} data.".format(n=args.dim,
                                                                                                 d=args.data))
    except:
        print('Done')
