...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="train_transr",
                                     description="Trains the TransR algorithm for kg embeddings")
    ...
    if args.data == 'riedel':
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "transr_{d}_riedel.ckpt".format(d=args.dim))
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "transr_{d}_gids.ckpt".format(d=args.dim))
        hyperparams = {'batch_size': 128,
                       'neg_ent': 25,
                       'neg_rel': 0,
                       'margin': 5.0,
    ...
    model = NegativeSampling(model=transr,
    ...
    trainer = Trainer(model=model,
                      ...
                      use_gpu=True)
    trainer.run()
    transr.save_checkpoint(out_path)
    transr.load_checkpoint(out_path)
    tester = Tester(model=transr, data_loader=test_dataloader, use_gpu=True)
    tester.run_link_prediction(type_constrain=True)
    try:
        text_results("Finished training TransR with {n} dimensional vectors on {d} data.".format(n=args.dim,
                                                                                                 d=args.data))
    except:
        print('Done')
