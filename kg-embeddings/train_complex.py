...
from OpenKE.openke.module.model import ComplEx
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="train_complex",
                                     description="Trains the ComplEx algorithm for kg embeddings")
    ...
    if args.data == 'riedel':
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "complex_{d}_riedel.ckpt".format(d=args.dim))
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "complex_{d}_gids.ckpt".format(d=args.dim))
        hyperparams = {'batch_size': 128,
                       'neg_ent': 25,
                       'neg_rel': 0,
                       'margin': 5.0,
    ...
    trainer = Trainer(model=model, data_loader=train_dataloader,
                      ...
                      opt_method="adagrad")
    trainer.run()
    complex.save_checkpoint(out_path)
    complex.load_checkpoint(out_path)
    tester = Tester(model=complex, data_loader=test_dataloader, use_gpu=True)
    tester.run_link_prediction(type_constrain=True)
    text_results("Finished training ComplEx with {n} dimensional vectors on {d} data.".format(n=args.dim, d=args.data))
