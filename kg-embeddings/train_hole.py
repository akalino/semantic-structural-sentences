...
from OpenKE.openke.module.model import HolE
...
if __name__ == "__main__":
    ...
    if args.data == 'riedel':
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "hole_{d}_riedel.ckpt".format(d=args.dim))
        ...
        out_path = os.path.join(wd, "kg-embeddings", "checkpoints", "hole_{d}_gids.ckpt".format(d=args.dim))
        hyperparams = {'batch_size': 128,
                       'neg_ent': 25,
                       'neg_rel': 0,
                       'margin': 5.0,
    ...
    model = NegativeSampling(model=hole_model,
    ...
    trainer = Trainer(model=model,
                      ...
                      use_gpu=True)
    trainer.run()
    hole_model.save_checkpoint(out_path)
    hole_model.load_checkpoint(out_path)
    tester = Tester(model=hole_model, data_loader=test_dataloader, use_gpu=True)
    tester.run_link_prediction(type_constrain=True)
    try:
        text_results("Finished training HolE with {n} dimensional vectors on {d} data.".format(n=args.dim,
                                                                                                 d=args.data))
    except:
        print('Done')
