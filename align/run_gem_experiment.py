...
from align_models.GemSpaceMapper import GemSpaceMapper
from align_models.CLR import CyclicLR
from align_models.GEMDataLoader import GEMDataLoader
...
def evaluate(_model, _valid_x, _valid_y, _shape, _valid_path, _w, _ann, _lm, _cn):
    ...
    if _w:
        rel_hits = valid_df.groupby('rel_idx').agg({'hits_5': 'sum', 'hits_10': 'sum'})
        print(rel_hits)
...
def run(_sent_space, _triple_space, _valid_triples, _test_triples,
        _agg, _norm,
        _train, _valid, _test, _batch_sz,
        _init_lr, _epochs, _es_tol, _beta, _tune):
    _model = GemSpaceMapper(_sent_space, _triple_space, _agg, _norm, _beta, _tune)
    ...
    val_prior = 10
    es_tol = 0
    data = GEMDataLoader(_train, _valid, _test, _batch_sz)
...
def experiment(_config_name):
    ...
    train_triple_emb_path = os.path.join(wd,
                                         'kg-embeddings/data/train_gem_triples_{e}_{d}_{a}.pt'.format(
                                             e=config['kg_embedding'],
                                             d=config['kg_dimension'],
                                             a=config['data_source']))
    valid_triple_emb_path = os.path.join(wd,
                                         'kg-embeddings/data/valid_gem_triples_{e}_{d}_{a}.pt'.format(
                                             e=config['kg_embedding'],
                                             d=config['kg_dimension'],
                                             a=config['data_source']))
    test_triple_emb_path = os.path.join(wd,
                                         'kg-embeddings/data/test_gem_triples_{e}_{d}_{a}.pt'.format(
                                             e=config['kg_embedding'],
                                             d=config['kg_dimension'],
                                             a=config['data_source']))
    train_path = os.path.join(wd, 'data/RESIDE/{d}_data/gem_training_set.csv'.format(d=config['data_source']))
    valid_path = os.path.join(wd, 'data/RESIDE/{d}_data/gem_validation_set.csv'.format(d=config['data_source']))
    test_path = os.path.join(wd, 'data/RESIDE/{d}_data/gem_testing_set.csv'.format(d=config['data_source']))
    sent_emb = torch.load(sent_emb_path)
    train_triples = torch.load(train_triple_emb_path)
    valid_triples = torch.load(valid_triple_emb_path)
    test_triples = torch.load(test_triple_emb_path)
    model, valid_x, valid_y = run(sent_emb,
